...
```{r bep01-task1, echo=FALSE}
c(
  rows = nrow(sharks),
  columns = ncol(sharks),
  deals = sum(sharks$deal_on_show),
  deal_proportion = mean(sharks$deal_on_show),
  median_description_words = median(sharks$description_words)
)
```
...
```{r bep01-task2, echo=FALSE}
D <- sharks$deal_on_show == 1
L <- sharks$season >= 9
c(P_D = mean(D), P_L = mean(L), P_D_and_L = mean(D & L))
```
...
```{r bep01-timed, echo=FALSE}
w <- sharks$description_words
c(
  mean = mean(w),
  median = median(w),
  minimum = min(w),
  q1 = unname(quantile(w, 0.25)),
  q3 = unname(quantile(w, 0.75)),
  maximum = max(w)
)
```
