...
<li>interpret quantiles and the interquartile range;</li>
...
You do not need calculus in this course. You need to interpret probability as
area and use a CDF, R, or supplied tables to calculate it.
...
A **quantile** is a boundary below which a chosen proportion of values lies.
For example:

- the first quartile \(Q_1\) has 25% of values at or below it;
- the median \(Q_2\) has 50% of values at or below it;
- the third quartile \(Q_3\) has 75% of values at or below it.
...
It measures the width of the middle 50% of the distribution. Unlike the full
range, it is not determined by the smallest and largest observations.


``` r
equity_quantiles <- quantile(
...
<img src="B06-continuous-distributions_files/figure-html/b06-equity-cdf-quantiles-1.png" width="672" />

The horizontal axis measures initial equity in percentage points; the vertical
axis gives the proportion of recorded pitches at or below each value. To read
\(Q_1\), begin at 0.25 on the vertical axis, move to the step graph, and then
move down to the horizontal axis. Here \(Q_1=8\), the median is 10, and
\(Q_3=20\). Therefore the IQR is \(20-8=12\) percentage points: the middle
half of recorded initial offers runs from 8% to 20%.
...
5. What does the IQR measure?
6. What does a z-score of \(-1.5\) mean?
...
5. The width of the middle 50% of a distribution: \(Q_3-Q_1\).
6. The value is 1.5 standard deviations below the mean.
...
4. Use `quantile(sharks$equity_offered_pct, c(0.25, 0.5, 0.75))` and
   `IQR(sharks$equity_offered_pct)`. Report and interpret the IQR.
5. Explain why the normal curve in the graph is not a convincing model for
   offered equity.
6. A uniform variable is recorded in minutes and then converted to seconds.
...
4. \(Q_1=8\), the median is 10, and \(Q_3=20\), so the IQR is
   \(20-8=12\) percentage points. The middle half of the recorded initial
   equity offers lies from 8% to 20%.
5. The empirical distribution is bounded, right-skewed, and concentrated at
   a few common percentages; the fitted normal curve does not reproduce those
   features.
6. The numerical support and density height change with the units. Probabilities
