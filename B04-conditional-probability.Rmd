...
period_deal <- table(
  Period = factor(L, levels = c(FALSE, TRUE),
                  labels = c("Seasons 1-8", "Seasons 9-16")),
...
```{r b04-same-mean-spread, echo=FALSE, fig.width=7, fig.height=3.15}
x_spread <- 0:2
p_narrow <- c(0.10, 0.80, 0.10)
p_wide <- c(0.45, 0.10, 0.45)
spread_variances <- c(0.20, 0.90)

old_par <- par(no.readonly = TRUE)
par(
  mfrow = c(1, 2),
  mar = c(4.0, 4.1, 2.7, 0.8),
  las = 1,
  family = "sans"
)
for (i in 1:2) {
  probs <- list(p_narrow, p_wide)[[i]]
  plot(
    NA,
    xlim = c(-0.35, 2.35), ylim = c(0, 0.9),
    axes = FALSE, bty = "n",
    xlab = "Possible value of X",
    ylab = if (i == 1) "Probability" else "",
    main = c("Distribution A", "Distribution B")[i],
    cex.main = 1.0, cex.lab = 0.95
  )
  mtext(
    paste0(
      "mean = 1    |    variance = ",
      sprintf("%.2f", spread_variances[i])
    ),
    side = 3, line = 0.15, cex = 0.76, col = "#4A5560"
  )
  abline(h = seq(0, 0.8, by = 0.2), col = "#E9EEF2", lwd = 1)
  rect(
    x_spread - 0.18, 0, x_spread + 0.18, probs,
    col = "#B9D8E8", border = "#0072B2", lwd = 1.5
  )
  axis(1, at = x_spread)
  if (i == 1) {
    axis(2, at = seq(0, 0.8, by = 0.2))
  }
  abline(v = 1, lty = 2, lwd = 1.7, col = "#D55E00")
}
par(old_par)
```
...
```{r b04-gender-deal-plot, echo=FALSE}
gender_counts <- table(
  factor(sharks$pitcher_gender_group[known_gender], levels = gender_labels)
)
bar_positions <- barplot(
  gender_deal_rates,
  names.arg = paste0(
    c("All-men", "All-women", "Mixed"),
    "\n(n = ", as.integer(gender_counts), ")"
  ),
  col = c("#0072B2", "#009E73", "#CC79A7"),
  border = NA,
  ylim = c(0, 0.75),
  axes = FALSE,
  xlab = "Source classification of pitching team",
  ylab = "Pitches with an on-air deal"
)
axis(2, at = seq(0, 0.75, by = 0.15),
     labels = paste0(seq(0, 75, by = 15), "%"), las = 1)
text(bar_positions, gender_deal_rates + 0.025,
     labels = paste0(round(100 * gender_deal_rates, 1), "%"))
```
...
```{r b04-total-probability-diagram, echo=FALSE, fig.width=7, fig.height=3.6}
p_early <- mean(!L)
...
rect(0, 0, p_early, 1, col = "#EAF2F8", border = NA)
rect(p_early, 0, 1, 1, col = "#FFF1DB", border = NA)
rect(0, 0, p_early, p_deal_early, col = "#0072B2", border = NA)
rect(p_early, 0, 1, p_deal_late, col = "#E69F00", border = NA)
rect(0, 0, 1, 1, border = "#17324D", lwd = 2)
segments(p_early, 0, p_early, 1, col = "#17324D", lwd = 2)
text(p_early / 2, 0.28, "D ∩ Lᶜ", col = "white", cex = 1.1)
text(p_early + (1 - p_early) / 2, 0.36, "D ∩ L",
     col = "#17324D", cex = 1.1)
text(p_early / 2, 0.88, expression(L^c), col = "#0072B2", cex = 1.15)
text(p_early + (1 - p_early) / 2, 0.88, "L",
     col = "#A65000", cex = 1.15)
text(0.99, 0.98, expression(Omega), adj = c(1, 1), col = "#17324D")
```
