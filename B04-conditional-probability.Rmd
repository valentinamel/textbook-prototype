...
```{r b04-same-mean-spread, echo=FALSE, fig.width=7, fig.height=3.3}
x_spread <- 0:2
p_narrow <- c(0.10, 0.80, 0.10)
p_wide <- c(0.45, 0.10, 0.45)

old_par <- par(no.readonly = TRUE)
par(mfrow = c(1, 2), mar = c(4.2, 4.2, 2.2, 0.8), las = 1)
for (i in 1:2) {
  probs <- list(p_narrow, p_wide)[[i]]
  plot(x_spread, probs, type = "h", lwd = 12, lend = 1,
       col = c("#0072B2", "#D55E00")[i],
       xlim = c(-0.35, 2.35), ylim = c(0, 0.9),
       xaxt = "n", bty = "n",
       xlab = "Possible value of X", ylab = "Probability",
       main = c("Distribution A", "Distribution B")[i])
  axis(1, at = x_spread)
  points(x_spread, probs, pch = 16, cex = 1.2,
         col = c("#0072B2", "#D55E00")[i])
  abline(v = 1, lty = 2, lwd = 2, col = "#17324D")
  text(1, 0.88, "E[X] = 1", pos = 4, cex = 0.9)
}
par(old_par)
```
