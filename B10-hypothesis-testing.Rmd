...
```{r b10-errors-power-plot, echo=FALSE}
z_grid <- seq(-4.5, 6.5, length.out = 1600)
critical <- qnorm(0.975)
null_density <- dnorm(z_grid, mean = 0)
alternative_density <- dnorm(z_grid, mean = 2.5)

old_par <- par(no.readonly = TRUE)
par(mfrow = c(1, 2), mar = c(4.4, 4.3, 1.5, 0.7), las = 1)

plot(z_grid, null_density, type = "l", lwd = 2.5, col = "#17324D",
     bty = "n", xlim = c(-4, 4), xlab = "z statistic",
     ylab = "Density", main = "Null is true")
left <- z_grid <= -critical
right <- z_grid >= critical & z_grid <= 4
polygon(c(-4, z_grid[left], -critical),
        c(0, null_density[left], 0), col = "#E69F00", border = NA)
polygon(c(critical, z_grid[right], 4),
        c(0, null_density[right], 0), col = "#E69F00", border = NA)
abline(v = c(-critical, critical), lty = 2, col = "#7A4A00")
text(0, 0.13, "do not reject", col = "#17324D")
text(c(-2.5, 2.5), c(0.018, 0.018), c("alpha / 2", "alpha / 2"),
     col = "#7A4A00", cex = 0.85)

plot(z_grid, alternative_density, type = "l", lwd = 2.5, col = "#0072B2",
     bty = "n", xlim = c(-4, 6), xlab = "z statistic",
     ylab = "Density", main = "An alternative value is true")
left_alt <- z_grid <= -critical
right_alt <- z_grid >= critical & z_grid <= 6
polygon(c(-4, z_grid[left_alt], -critical),
        c(0, alternative_density[left_alt], 0),
        col = "#009E73", border = NA)
polygon(c(critical, z_grid[right_alt], 6),
        c(0, alternative_density[right_alt], 0),
        col = "#009E73", border = NA)
abline(v = c(-critical, critical), lty = 2, col = "#176B52")
text(3.6, 0.14, "power", col = "#17324D")
par(old_par)
```
