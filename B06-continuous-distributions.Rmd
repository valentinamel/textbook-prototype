...
plot(
  ...
  xaxs = "i",
  yaxs = "i",
  xlab = "Possible value of X",
  ylab = "Density"
)
polygon(
  c(a, a, b, b),
  c(0, density_height, density_height, 0),
  col = "#DCEAF3",
  border = NA
)
polygon(
  c(c_cut, c_cut, d_cut, d_cut),
  c(0, density_height, density_height, 0),
  col = "#E69F00",
  border = NA
)
lines(
  c(7, a, a, b, b, 33),
  c(0, 0, density_height, density_height, 0, 0),
  col = "#17324D",
  lwd = 3
)
axis(1, at = c(a, c_cut, d_cut, b),
     labels = c("a = 10", "c = 14", "d = 20", "b = 30"))
axis(2, at = c(0, 0.025, 0.05),
     labels = c("0", "0.025", "0.05"), las = 1)
text(20, 0.056, "density = 1 / (b - a) = 0.05", col = "#17324D")
arrows(17, 0.018, 17, 0.003, length = 0.08, col = "#7A4A00")
text(17, 0.023, "area = P(14 < X < 20) = 0.30", col = "#7A4A00")
```
...
```{r b06-normal-shape, echo=FALSE}
z <- seq(-3.6, 3.6, length.out = 800)
normal_density <- dnorm(z)
plot(
  z,
  normal_density,
  type = "l",
...
polygon(
  c(-1, z[inside], 1),
  c(0, normal_density[inside], 0),
  col = grDevices::adjustcolor("#009E73", alpha.f = 0.35),
  border = NA
)
lines(z, normal_density, lwd = 3, col = "#17324D")
abline(v = c(-1, 0, 1), lty = c(2, 1, 2),
       col = c("#666666", "#D55E00", "#666666"), lwd = c(1.5, 2, 1.5))
axis(1, at = c(-2, -1, 0, 1, 2),
     labels = c(expression(mu-2*sigma), expression(mu-sigma),
                expression(mu), expression(mu+sigma),
                expression(mu+2*sigma)))
text(0, 0.16, "about 68%", col = "#17324D")
```
...
```{r b06-equity-normal-check, echo=FALSE}
x <- sharks$equity_offered_pct
hist(
  x,
...
curve(
  dnorm(x, mean = mean(sharks$equity_offered_pct),
        sd = sd(sharks$equity_offered_pct)),
  add = TRUE,
  col = "#D55E00",
  lwd = 3
)
legend(
  "topright",
  c("Observed pitches", "Normal curve with same mean and SD"),
  pch = c(15, NA), lty = c(0, 1), lwd = c(NA, 3),
  col = c("#DCEAF3", "#D55E00"), pt.cex = 2,
  bty = "n"
)
```
...
```{r b06-equity-cdf-quantiles, echo=FALSE, fig.width=7, fig.height=4.2}
equity_sorted <- sort(sharks$equity_offered_pct)
equity_cumulative <- seq_along(equity_sorted) / length(equity_sorted)
q_values <- as.numeric(equity_quantiles)
q_probs <- c(0.25, 0.50, 0.75)

plot(
  equity_sorted, equity_cumulative,
  type = "s", lwd = 2.5, col = "#17324D",
  xlim = c(0, 100), ylim = c(0, 1),
  bty = "n", las = 1,
  xlab = "Equity initially offered (%)",
  ylab = "Cumulative proportion"
)
segments(0, q_probs, q_values, q_probs,
         col = c("#0072B2", "#009E73", "#D55E00"),
         lty = 2, lwd = 2)
segments(q_values, 0, q_values, q_probs,
         col = c("#0072B2", "#009E73", "#D55E00"),
         lty = 2, lwd = 2)
points(q_values, q_probs, pch = 19,
       col = c("#0072B2", "#009E73", "#D55E00"), cex = 1.1)
legend(
  "bottomright",
  legend = c("Q1 = 8%", "Median = 10%", "Q3 = 20%"),
  col = c("#0072B2", "#009E73", "#D55E00"),
  lty = 2, pch = 19, bty = "n"
)
```
