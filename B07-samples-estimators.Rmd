...
```{r b07-population-sample-plot, echo=FALSE}
breaks <- seq(0, 100, by = 5)
pop_hist <- hist(sharks$equity_offered_pct, breaks = breaks, plot = FALSE)
samp_hist <- hist(pitch_sample$equity_offered_pct, breaks = breaks,
                  plot = FALSE)
...
plot(
  ...
  ylab = "Density",
  ylim = c(0, max(pop_hist$density, samp_hist$density) * 1.12)
)
lines(samp_hist$mids, samp_hist$density, type = "b", pch = 21,
      bg = "#E69F00", col = "#7A4A00", lwd = 2)
legend("topright", c("All 1,441 records", "Random sample of 100"),
       col = c("#9ECAE1", "#7A4A00"), lwd = c(8, 2),
       pch = c(NA, 21), pt.bg = c(NA, "#E69F00"), bty = "n")
```
