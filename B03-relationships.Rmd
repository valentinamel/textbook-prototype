...
```{r b03-season-pmf-plot, echo=FALSE}
barplot(
  season_pmf,
  col = "#0072B2", border = NA,
  xlab = "Season s", ylab = expression(p[S](s)),
  ylim = c(0, 1.18 * max(season_pmf))
)
```
...
```{r b03-season-cdf, echo=FALSE}
plot(
  season_distribution$season, season_distribution$cdf,
  type = "s", lwd = 3, col = "#0072B2",
  xlab = "Threshold a", ylab = expression(F[S](a)),
  ylim = c(0, 1)
)
points(
  season_distribution$season, season_distribution$cdf,
